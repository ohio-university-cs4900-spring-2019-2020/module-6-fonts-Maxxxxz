...
GLViewModule_6* GLViewModule_6::New( const std::vector< std::string >& args )
{
   GLViewModule_6* glv = new GLViewModule_6( args );
...
GLViewModule_6::GLViewModule_6( const std::vector< std::string >& args ) : GLView( args )
{
   //Initialize any member variables that need to be used inside of LoadMap() here.
   //Note: At this point, the Managers are not yet initialized. The Engine initialization
   //occurs immediately after this method returns (see GLViewModule_6::New() for
   ...
   //GLViewModule_6::onCreate() is invoked after this module's LoadMap() is completed.
}


void GLViewModule_6::onCreate()
{
   //GLViewModule_6::onCreate() is invoked after this module's LoadMap() is completed.
...
void GLViewModule_6::updateWorld()
{
...
void GLViewModule_6::onResizeWindow( GLsizei width, GLsizei height )
{
   GLView::onResizeWindow( width, height ); //call parent's resize method.
}


void GLViewModule_6::onMouseDown( const SDL_MouseButtonEvent& e )
{
   GLView::onMouseDown( e );
}


void GLViewModule_6::onMouseUp( const SDL_MouseButtonEvent& e )
{
   GLView::onMouseUp( e );
}


void GLViewModule_6::onMouseMove( const SDL_MouseMotionEvent& e )
{
   GLView::onMouseMove( e );
}


void GLViewModule_6::onKeyDown( const SDL_KeyboardEvent& key )
{
...
void GLViewModule_6::onKeyUp( const SDL_KeyboardEvent& key )
{
   GLView::onKeyUp( key );
}

void GLViewModule_6::beginMessageSend()
{
	std::cout << "hello" << std::endl;
	std::string usrmsg;
	std::getline(std::cin, usrmsg);
	std::cout << "MESSAGE: " << usrmsg << std::endl;

	Module6NetMsg msg(usrmsg, this->getCamera()->getPosition());
	client->sendNetMsgSynchronousTCP(msg);

}

void GLViewModule_6::createTextAtPos(std::string msg, Aftr::Vector pos)
{
	//create text at pos + z+10
	std::cout << "Creating text at position " << pos << std::endl;
	std::string comic = ManagerEnvironmentConfiguration::getSMM() + "fonts/COMIC.TTF";
	WOFTGLString* label = WOFTGLString::New(comic, 30);
	label->setText(msg);
	label->getModelT<MGLFTGLString>()->setFontColor(Aftr::aftrColor4f(255.0f, 0.0f, 0.0f, 255.0f));
	label->getModelT<MGLFTGLString>()->setText(msg);
	label->getModelT<MGLFTGLString>()->rotateAboutGlobalX(Aftr::PI / 2);
	label->getModelT<MGLFTGLString>()->setSize(30, 10);
	label->setPosition(Vector(pos.x, pos.y, pos.z + 20));
	worldLst->push_back(label);
}

void Aftr::GLViewModule_6::loadMap()
{
   ...
   createModule_6WayPoints();
}


void GLViewModule_6::createModule_6WayPoints()
{
